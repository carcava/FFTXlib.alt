...
CONTAINS
  ...
  SUBROUTINE fw_scatter( iopt )
     ...
     IF( iopt == 2 ) THEN
        ...
        IF( use_tg ) THEN
           !
           CALL fft_scatter( dfft, aux, nx3, dfft%nogrp*dfft%tg_nnr, f, dfft%tg_nsw, dfft%tg_npp, iopt )
           !
        ELSE
           !
...
SUBROUTINE fw_tg_cft3_scatter( f, dfft, aux )
  ...
  TYPE (fft_type_descriptor), INTENT(in) :: dfft     ! descriptor of fft data layout
  !
  CALL fft_scatter( dfft, aux, dfft%nr3x, dfft%nogrp*dfft%tg_nnr, f, dfft%tg_nsw, dfft%tg_npp, 2 )
  !
